...
func (c *Compiler) buildJobLevelSafeOutputEnvVars(data *WorkflowData, workflowID string) map[string]string {
	...
	// Set GH_AW_CALLER_WORKFLOW_ID to the workflow's runtime repository + compile-time workflow ID.
	// Using the workflow ID (filename without extension) instead of github.workflow (which returns
	// the display name like "CLI Version Checker" or the file path when no name is set) ensures
	// consistent, stable identifiers for issue/discussion markers and search queries.
