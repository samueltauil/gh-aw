...
async function main(config = {}) {
  ...
  return async function handleCreateDiscussion(message, resolvedTemporaryIds) {
    ...
    // GH_AW_CALLER_WORKFLOW_ID is set at runtime to `github.workflow_ref`
    // (e.g. "owner/repo/.github/workflows/caller.lock.yml@refs/heads/main").
    ...
    // with the caller's workflow file identity so close-older-discussions can distinguish callers precisely.
