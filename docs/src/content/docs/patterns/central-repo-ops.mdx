...
```mermaid
graph TB
    subgraph Central["Central Control Repository"]
        O["Orchestrator Workflow\n(selects & prioritizes repos)"]
        W["Worker Workflow\n(runs per target repo)"]
    end

    O -->|"dispatch-workflow\n(max: N)"| W

    W -->|"checkout + create PR/issue"| R1["Target Repo 1"]
    W -->|"checkout + create PR/issue"| R2["Target Repo 2"]
    W -->|"checkout + create PR/issue"| R3["Target Repo 3"]
    W -->|"checkout + create PR/issue"| RN["Target Repo N"]
```
